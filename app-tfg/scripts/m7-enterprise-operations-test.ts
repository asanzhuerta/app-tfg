...
async function main() {
	...
	try {
		...
		const proposal = await generateSupplierOrderProposal({
			...
			quantity: 2,
			reason: "test_reposición",
			notes: "Test temporal M7 enterprise operations",
			status: "generated",
		});
...
main()
	.then(() => {
		console.log("M7 enterprise operations test OK");
	})
	.catch((error) => {
		console.error("M7 enterprise operations test FAILED");
		console.error(error);
		process.exitCode = 1;
	});
