...
async function main() {
	...
	const tag = `M5 technical email smoke ${Date.now()}`;
	...
	assertCondition(
		...
		"La limpieza final del smoke de M5 ha dejado fichas temporales sin borrar",
	);
...
void main()
	.then(() => {
		console.log("M5 technical email smoke OK");
	})
	.catch((error) => {
		console.error("M5 technical email smoke FAILED");
		console.error(error);
		process.exitCode = 1;
	});
