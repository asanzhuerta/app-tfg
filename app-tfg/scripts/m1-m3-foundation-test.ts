...
async function main() {
	...
	const now = Date.now();
	const tag = `m1-m3-foundation-${now}`;
	...
	try {
		...
		console.log("OK M1-M3 foundation test: 11/11 comprobaciones superadas");
	} finally {
...
main().catch((error) => {
	console.error("FAIL M1-M3 foundation test");
	console.error(error);
	process.exit(1);
});
