...
async function loadTemplateLines() {
	...
	assertCondition(
		...
		"No hay un pedido base con lineas para reutilizar en la prueba de pagos parciales",
	);
...
async function main() {
	...
	const tag = `M4 payments test ${Date.now()}`;
	...
	try {
		...
		console.log(`M4 payments test OK (${candidate.commercialName})`);
	} finally {
...
void main().catch((error) => {
	console.error("M4 payments test FAILED");
	console.error(error);
	process.exitCode = 1;
});
