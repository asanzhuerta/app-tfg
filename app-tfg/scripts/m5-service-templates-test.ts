...
async function main() {
	...
	const tag = `M5 service template test ${Date.now()}`;
	...
	assertCondition(
		...
		"La limpieza final de la prueba de plantillas de M5 ha dejado registros temporales",
	);
...
void main()
	.then(() => {
		console.log("M5 service templates test OK");
	})
	.catch((error) => {
		console.error("M5 service templates test FAILED");
		console.error(error);
		process.exitCode = 1;
	});
