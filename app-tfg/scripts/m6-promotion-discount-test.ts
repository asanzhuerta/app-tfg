...
// Usamos un descuento alto para que la prueba no falle por promociones reales.
...
async function main() {
	...
	const now = Date.now();
	const tag = `m6-discount-test-${now}`;
...
void main()
	.then(() => {
		console.log("M6 promotion discount test OK");
	})
	.catch((error) => {
		console.error("M6 promotion discount test FAILED");
		console.error(error);
		process.exitCode = 1;
	});
