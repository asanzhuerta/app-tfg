...
En este apartado se describe el modelo conceptual correspondiente al modulo de pedidos, entregas y cobros.

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\textwidth]{figures/MCD/M4_ModeloConceptual.pdf}
%     \caption{Modelo conceptual de datos del modulo M4.}
%     \label{fig:mcd-m4}
% \end{figure}

\subsubsection{Clase Pedido}

La clase \textit{Pedido} representa cada solicitud de compra realizada por un cliente profesional o por un usuario autorizado en su nombre. En ella se almacena la informacion necesaria para identificar el pedido, conocer su estado dentro del ciclo comercial, vincularlo opcionalmente a un reparto y mantener su seguimiento minimo de cobro.
...
\begin{itemize}
      ...
      \item \textit{payment\_status}
            Estado actual de cobro del pedido.
      \item \textit{delivery\_visit}
            Visita de reparto vinculada, cuando exista.
      \item \textit{payment\_method}
            Metodo de cobro registrado, cuando exista.
      \item \textit{payment\_notes}
            Notas de cobro, cuando existan.
      \item \textit{paid\_at}
            Fecha en la que se marco el cobro, cuando exista.
      \item \textit{paid\_by\_user}
            Usuario que registro el cambio de cobro, cuando exista.
      ...
      \item \textit{updated\_at}
            Fecha de ultima actualizacion del pedido.
\end{itemize}

\textbf{Restricciones textuales}

\begin{itemize}
      \item \textbf{RT-O1 --- Asociacion obligatoria del pedido}
            Todo pedido debe estar asociado a un cliente existente y a un usuario valido que lo registre.
      \item \textbf{RT-O2 --- Estado valido del pedido}
            El estado de un pedido solo puede tomar valores definidos por el sistema y respetar las transiciones permitidas del flujo implementado.
      \item \textbf{RT-O3 --- Limite de pedidos abiertos sin cobrar}
            Un mismo cliente no debe acumular mas de dos pedidos abiertos pendientes de cobro.
      \item \textbf{RT-O4 --- Vinculacion logistica coherente}
            Solo un pedido confirmado puede vincularse a una visita de reparto activa.
      \item \textbf{RT-O5 --- Coherencia de entrega}
            Un pedido solo puede pasar a \texttt{delivered} al completarse correctamente la visita de reparto vinculada.
      \item \textbf{RT-O6 --- Coherencia de cobro}
            Un pedido solo puede pasar a \texttt{paid} si previamente se encuentra entregado y dispone del registro minimo de cobro asociado.
\end{itemize}

\subsubsection{Clase Linea de pedido}

La clase \textit{Linea de pedido} representa cada uno de los productos incluidos en un pedido. Su finalidad es detallar la composicion del pedido, las cantidades solicitadas y la instantanea comercial consolidada en el momento de crear la linea.

\textbf{Atributos}

\begin{itemize}
      \item \textit{id}
            Identificador unico de la linea de pedido.
      \item \textit{order}
            Pedido asociado a la linea.
      \item \textit{product}
            Producto incluido en la linea.
      \item \textit{color\_reference}
            Referencia cromatica asociada, cuando proceda.
      \item \textit{quantity}
            Cantidad solicitada del producto.
      \item \textit{unit\_price\_snapshot}
            Precio unitario consolidado en el momento del pedido.
      \item \textit{discount\_percentage}
            Porcentaje de descuento aplicado a la linea.
      \item \textit{line\_total}
            Importe total de la linea.
      \item \textit{order\_reference\_snapshot}
            Referencia comercial consolidada del producto.
      \item \textit{variant\_code\_snapshot}
            Codigo de variante consolidado, cuando exista.
      \item \textit{variant\_name\_snapshot}
            Nombre de variante consolidado, cuando exista.
\end{itemize}

\textbf{Restricciones textuales}

\begin{itemize}
      \item \textbf{RT-OL1 --- Asociacion obligatoria de la linea}
            Toda linea de pedido debe estar asociada a un pedido y a un producto existentes.
      \item \textbf{RT-OL2 --- Cantidad valida}
            Toda linea de pedido debe registrar una cantidad solicitada valida y mayor que cero.
      \item \textbf{RT-OL3 --- Importe coherente de linea}
            El importe total de la linea debe ser coherente con la cantidad, el precio consolidado y el descuento aplicado.
      \item \textbf{RT-OL4 --- Trazabilidad comercial de la linea}
            La linea debe conservar la instantanea minima necesaria para seguir identificando el producto pedido aunque cambien metadatos posteriores del catalogo.
\end{itemize}

\subsubsection{Clase Estado de pedido}

La clase \textit{Estado de pedido} representa el catalogo controlado de estados que puede adoptar un pedido dentro del flujo implementado.
...
\begin{itemize}
      \item \textbf{RT-OS1 --- Unicidad del estado}
            Cada estado de pedido debe disponer de un codigo y un nombre unicos dentro del sistema.
\end{itemize}

\subsubsection{Clase Estado de cobro del pedido}

La clase \textit{Estado de cobro del pedido} representa el catalogo controlado de estados de pago asociado al seguimiento minimo de cobros implementado en \texttt{M4}.

\textbf{Atributos}

\begin{itemize}
      \item \textit{id}
            Identificador unico del estado de cobro.
      \item \textit{code}
            Codigo interno del estado de cobro.
      \item \textit{name}
            Nombre visible del estado de cobro.
\end{itemize}

\textbf{Restricciones textuales}

\begin{itemize}
      \item \textbf{RT-OPS1 --- Unicidad del estado de cobro}
            Cada estado de cobro debe disponer de un codigo y un nombre unicos dentro del sistema.
\end{itemize}

\subsubsection{Clase Visita comercial de reparto vinculada}

La implementacion actual reutiliza la clase de visita comercial del modulo \texttt{M2} para modelar la fase logistica del pedido. Cuando una visita es de tipo reparto, puede actuar como soporte logistico de uno o varios pedidos del mismo cliente.

\textbf{Atributos}

\begin{itemize}
      \item \textit{id}
            Identificador unico de la visita de reparto.
      \item \textit{client}
            Cliente asociado a la visita.
      \item \textit{commercial}
            Comercial responsable del reparto.
      \item \textit{scheduled\_for\_date}
            Fecha prevista del reparto.
      \item \textit{visit\_type}
            Tipo de visita, que debe corresponder a reparto.
      \item \textit{status}
            Estado actual de la visita.
      \item \textit{notes}
            Observaciones asociadas a la visita.
      \item \textit{result}
            Resultado operativo de la visita, cuando exista.
\end{itemize}

\textbf{Restricciones textuales}

\begin{itemize}
      \item \textbf{RT-DV1 --- Tipo de visita compatible}
            Solo una visita comercial de tipo reparto puede vincularse logisticamente a pedidos del modulo \texttt{M4}.
      \item \textbf{RT-DV2 --- Coherencia de cliente}
            Todos los pedidos vinculados a una visita de reparto deben pertenecer al mismo cliente asociado a dicha visita.
      \item \textbf{RT-DV3 --- Minimo de pedidos confirmados en reparto activo}
            Mientras una visita de reparto permanezca activa, debe mantener al menos un pedido confirmado vinculado.
      \item \textbf{RT-DV4 --- Cierre con validacion QR}
            Para completar un reparto, debe aportarse evidencia QR suficiente para todos los pedidos vinculados.
\end{itemize}
% ------------------------------------------------------------------------------
...
En este apartado se describe el modelo conceptual correspondiente al módulo de funcionalidades adicionales.
