...
\begin{itemize}
    ...
    \item \texttt{npx -y react-doctor@latest app --verbose --no-score}
...
\hline
\texttt{RF-M6-01}, \texttt{RF-M6-02} & \texttt{M6} & \path{/admin/communications}, \path{/clients/promotions}, \path{/commercials/promotions}, \path{/admin/settings} & \path{/api/admin/communications/promotions}, \path{/api/admin/communications/segments}, \path{/api/admin/settings/client-tiers}, \path{/api/admin/settings/client-tiers/recalculate}, \path{/api/communications/promotions} & \texttt{promotions}, \texttt{promotion\_discount\_types}, \texttt{customer\_segments}, \texttt{client\_customer\_segments}, \texttt{system\_configurations}, \texttt{app\_notifications} & \texttt{m6:communications-smoke} y \texttt{m6:promotion-discount-smoke}: promoción segmentada, visibilidad por rol y descuento aplicado a pedido. & Salidas \texttt{M6 communications smoke OK}, \texttt{M6 promotion discount smoke OK}; comprobación visual de comunicaciones, jerarquía de rangos, descuento, política de umbrales y recálculo manual. \\
\hline
