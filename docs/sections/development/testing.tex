...
El resultado de esta validación es satisfactorio: la comprobación de tipos, el análisis estático, la construcción completa de producción y la aplicación de migraciones finalizan sin errores, y el cierre técnico de \texttt{M4} queda reforzado por dos evidencias adicionales:
...
\begin{itemize}
    ...
    \item \texttt{M7}: consulta administrativa de auditoría, exportación \texttt{CSV} de accesos y acciones administrativas, configuración y diagnóstico de rate limiting, inventario operativo de integraciones de soporte, panel de soporte técnico/PWA, resumen ejecutivo de operación transversal, menú agrupado de configuración, informe operativo exportable, registro persistente de integraciones empresariales, operaciones de intercambio y propuestas de pedido a proveedor.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{|p{0.11\linewidth}|X|X|}
        ...
        \hline
        \texttt{M2} & Clientes, asignaciones, visitas, rutas, mapa, avisos por visitas creadas, aplazadas o reubicadas y ETA visible para cliente cuando existe reparto real. & Validación manual en navegador, barrido headless, pruebas cruzadas con el flujo de reparto de \texttt{M4} y comprobación estática de los nuevos servicios de notificación de visita. \\
        \hline
        ...
        \hline
        \texttt{M6} & Promociones, segmentos, badge de rango comercial, descuentos aplicados al pedido, formaciones, inscripciones, notificaciones internas segmentadas, correo SMTP, Web Push PWA y recordatorios. & Typecheck, lint, build, \texttt{migration:run}, \texttt{m6:communications-smoke}, \texttt{m6:promotion-discount-smoke}, \texttt{m6:client-tier-smoke}, \texttt{m6:closeout} y validación manual en navegador local con datos temporales de promoción, pedido con descuento, rango comercial y formación. La ampliación multicanal se ha comprobado en esta iteración con typecheck, lint y build. \\
        \hline
        \texttt{M7} & Auditoría administrativa, exportación \texttt{CSV} de trazabilidad, configuración y diagnóstico de rate limiting, inventario de integraciones de soporte, soporte técnico/PWA, resumen de operación transversal, menú agrupado de configuración, informe operativo exportable, registro empresarial de integraciones y propuestas a proveedor. & Typecheck, lint, build, \texttt{migration:run}, \texttt{m7:integrations-smoke}, \texttt{m7:support-smoke}, \texttt{m7:operations-smoke}, \texttt{m7:audit-export-smoke}, \texttt{m7:rate-limit-smoke}, \texttt{m7:operations-report-smoke}, \texttt{m7:enterprise-operations-smoke}, \texttt{m7:closeout} y revisión de \path{/admin/audit}, \path{/admin/operations}, \path{/admin/enterprise-operations}, \path{/admin/integrations}, \path{/admin/support} y \path{/admin/settings}. \\
        \hline
    \end{tabularx}
