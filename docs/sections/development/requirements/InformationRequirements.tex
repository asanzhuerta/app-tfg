...
El sistema debe almacenar la informacion general asociada a cada pedido realizado por un cliente profesional o por un usuario autorizado en su nombre. Esta informacion permite identificar el pedido, conocer su situacion dentro del flujo comercial y relacionarlo tanto con su ciclo logistico como con su situacion minima de cobro.
...
\begin{itemize}
    ...
    \item Fecha de ultima actualizacion del pedido.
\end{itemize}

\subsubsection*{Linea de pedido}

El sistema debe almacenar el detalle de los productos incluidos en cada pedido. Esta informacion permite conocer que productos se han solicitado, en que cantidades y con que instantanea comercial se consolidaron en el momento del pedido.

Los datos asociados a una linea de pedido incluyen:

\begin{itemize}
    \item Identificador de la linea de pedido.
    \item Pedido asociado.
    \item Producto incluido en la linea.
    ...
    \item Porcentaje de descuento aplicado a la linea.
    \item Importe total de la linea.
    \item Referencia comercial consolidada del producto.
    \item Codigo y nombre de variante consolidados, cuando existan.
\end{itemize}

\subsubsection*{Estado de pedido}

El sistema debe gestionar un catalogo controlado de estados de pedido para representar el ciclo de vida permitido de cada encargo.

Los datos asociados a un estado de pedido incluyen:

\begin{itemize}
    \item Identificador del estado.
    \item Codigo interno del estado.
    \item Nombre visible del estado.
\end{itemize}

\subsubsection*{Estado de cobro del pedido}

El sistema debe gestionar un catalogo controlado de estados de cobro para representar el seguimiento minimo del pago asociado al pedido.
...
La implementacion actual reutiliza la entidad de visita comercial del modulo \texttt{M2} para modelar la fase logistica del pedido. Cuando un pedido queda asociado a un reparto, la relacion se almacena mediante una visita comercial de tipo entrega o reparto.
...
En la implementacion actual no existe una entidad de cobro independiente ni una entidad de entrega separada del subsistema de visitas comerciales. El seguimiento minimo del cobro se resuelve sobre el propio pedido y la fase logistica reutiliza visitas comerciales de tipo reparto.
% ------------------------------------------------------------------------------
\subsection{RI-M5}
% ------------------------------------------------------------------------------
